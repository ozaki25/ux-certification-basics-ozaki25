...
| ✅ UXインテリジェンスの定義文言 | lesson05 | UXIA 側の公式発信「UXインテリジェンスとは、デジタル前提時代により善い社会をつくるために、あらゆる提供者がもつべき『能力』と『精神』」と一致（本文は同義のパラフレーズ）。設立時定義「より自由で豊かなUXを企画する精神と能力」とも整合。修正不要 | [電通デジタル 記事](https://www.dentsudigital.co.jp/knowledge-charge/articles/2021/1109-001093)、[ビービット プレスリリース](https://prtimes.jp/main/html/rd/p/000000071.000025505.html)、[日経ビジネススクール](https://school.nikkei.co.jp/ondemand/seminar/article?sid=E1000151) |
| ✅ 行動フロー・行動ギャップの定義 | lesson21 | 『UXグロースモデル』のシーケンス分析の説明（想定行動フローを書き出し、実際の行動データとの差分＝行動ギャップを発見して理由を推察する）と一致。修正不要 | [beBit 限定公開記事](https://afterdigital.bebit.co.jp/basic/article/basic_10)、[読書メモ（zenn）](https://zenn.dev/ama_o255/scraps/aba681da0a6f1f)、[しのジャッキー note まとめ6](https://note.com/shinojackie/n/n069cd2ec0876) |
| ✅ VPC 各要素の表記 | lesson23, q511/q512 | 「顧客プロファイル」は邦訳（翔泳社・関美和訳）の目次表記と一致し現状維持。「ゲインクリエーター」は講座資料・書籍由来の解説の主流表記に合わせて「ゲインクリエイター」に統一（lesson23・chapter5.ts を修正済み） | [翔泳社 書籍ページ](https://www.shoeisha.co.jp/book/detail/9784798140568)、[NDLサーチ 書誌](https://ndlsearch.ndl.go.jp/books/R100000002-I026299996)、[UX TIMES 用語集](https://uxdaystokyo.com/articles/glossary/whats_value-proposition-canvas/) |
...
| 項目 | 対象 | 現在の記述 | 確認先 |
|---|---|---|---|
| ⬜ 情報設計の3ステップと順序 | lesson25 | 組織化→構造化→ラベリングの3ステップを「順序とセットで覚える」と提示（教材独自の整理）。2026-07 の Web 調査で、古典的 IA（Rosenfeld/Morville）は組織化・ラベリング・ナビゲーション・検索の4システムであり、3ステップを順序付きで示す公開の確立典拠は見当たらないことを確認。一般的な IA 実務（分類→階層化→命名）とは矛盾しないため現状維持。順序を問うドリルも含め、講座資料・シラバスの再照合待ち | 公式シラバス「情報設計」行、講座資料（references/ 再アップロード時に照合） |
